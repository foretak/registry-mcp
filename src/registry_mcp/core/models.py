...
class CompanyReport(_Base):
    ...
    financials: FinancialSummary | None = Field(
        ...
        description=(
            ...
            "fetch failed (see `notes` for which attachment and why). Norway returns a "
            "*present* block with `periods: []` for an entity Regnskapsregisteret holds no "
            "filed accounts for. Sweden instead returns *no block at all*, plus a "
            "report-level `notes` sentence naming the reason, because Bolagsverket's "
            "digital annual-report channel holds nothing for that entity — a fact about "
            "the company, not about the country (D-042(d)(3), `tasks/T55.md`). Norway and "
            "Sweden today: "
