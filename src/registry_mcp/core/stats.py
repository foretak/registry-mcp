...
def summary(db_path: str | Path | None = None) -> dict[str, Any]:
    ...
    Returns:
        ...
        "none" bucket), ``top_queries`` (top 20 ``{"query", "count"}``,
    ...
    try:
        rows = conn.execute(
            ...
            "latency_ms, ok, cached FROM calls"
        ).fetchall()
    except Exception:
    ...
    for ts, surface, operation, country, query, user_agent, latency_ms, ok, cached in rows:
