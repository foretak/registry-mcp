...
def rules_markdown() -> str:
    ...
    return (
        ...
        "## Electronic invoicing (2027) and electronic bookkeeping (2030)\n"
        "From **1 January 2027**, every bokføringspliktig Norwegian business "
        "must issue B2B sales documents in an electronic invoice format to "
        "a counterparty 'som er registrert i Elektronisk mottakerregister "
        "(ELMA), og som dermed kan motta e-faktura' — i.e. registered in "
        "the Electronic Recipient Register and therefore able to receive "
        "an e-invoice (bokføringsloven § 10 new second paragraph, "
        "LOV-2026-06-19-39; Prop. 44 L (2025–2026) ch. 3). From **1 January "
        "2030**, the same businesses must also be able to *receive* "
        "electronic invoices and keep their books in an electronic "
        "accounting system (bokføringsloven § 7 new fourth paragraph). "
        "**Exempt**: businesses with turnover below NOK 50,000, and "
        "*finansvirksomhet, forsikringsselskaper og pensjonsforetak* (Prop. "
        "44 L ch. 3). Neither date is computed here as a `Deadline`: the "
        "duty carries no per-entity due date, and whether it binds a given "
        "entity turns on facts this register does not publish (turnover, "
        "sector) — guessing one from a legal form is exactly what this "
        "module declines to do elsewhere too. Call `lookup_company` with "
        "`include=[\"peppol\"]` for whether a specific counterparty is "
        "reachable on the Peppol network today, which is a different "
        "question from whether the 2027/2030 duty applies to it.\n\n"
