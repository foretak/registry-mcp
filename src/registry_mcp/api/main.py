...
_DESCRIPTION = (
    ...
    "and peppol — each a second fetch with its own provenance, null unless you ask.\n\n"
...
async def get_company(
    ...
    include: Sequence[str] = Query(
        ...
        description=(
            "Attachment names to fetch alongside the base report; empty by default, "
            "which costs exactly one upstream request. Each is a second, independent "
            "fetch attached at that same key on the result, with its own provenance, "
            "null unless you ask, and this parameter is where each of the seven is "
            "explained. `filings` (every country): what the entity has filed, and "
            "when — Companies House the whole filing history, Bolagsverket the filed "
            "annual reports, Regnskapsregisteret the filed annual accounts; the "
            "block's `notes` says which, and `total_count` how many more the register "
            "holds. `charges` (GB): registered mortgages and other security "
            "interests, in the register's own words. `insolvency` (GB): winding-up "
            "and administration proceedings — a members' voluntary liquidation is a "
            "*solvent* wind-up, so `is_liquidation: true` is not by itself evidence "
            "of distress. `financials` (NO and SE): the register's own figures for "
            "the latest filed accounting period — turnover, operating result, "
            "profit, balance sheet, equity, liabilities, each beside its currency, "
            "never a ratio or a verdict. Norway's come in the `filings` fetch, "
            "Sweden's out of the entity's own filed K2 annual report. A `null` "
            "figure in a present block means the company did not report that line; "
            "an absent block means you did not ask, the fetch failed, or — Sweden — "
            "it has filed no digital annual report, and `notes` says which. Britain "
            "does not declare it, so `financials` for GB is a `bad_request` (400), "
            "never an empty block. `lei` (every country except Sweden, whose "
            "identifier can be a natural person's): the Legal Entity Identifier "
            "GLEIF, the Global LEI Foundation, publishes — CC0 and keyless; `lei: "
            "null` in a present block means GLEIF holds none. `parents` (same "
            "countries as `lei`): the direct and ultimate parent from GLEIF's Level "
            "2 data — the entity that consolidates this one's accounts into its "
            "group, not necessarily its majority shareholder. Where GLEIF discloses "
            "none, that side carries the entity's own stated reason as a category "
            "word such as `NATURAL_PERSONS` — never a name, and unverified. "
            "`peppol` (Norway): whether an e-invoice can reach the entity over the "
            "Peppol network, read live from the SML/SMP walk the way ELMA resolves "
            "it, ahead of the 1 January 2027 EHF (Peppol BIS Billing 3.0) duty. "
            "`registered: null` means no authoritative answer — never read it as "
            "\"no\"; only an NXDOMAIN or an SMP 404 earns `false`. Repeat the "
            "parameter for more than one, e.g. `?include=filings&include=charges`. "
            "Read a country's `supported_includes` at `GET /v1/countries` first: a "
            "value it does not declare is a `bad_request` (400) naming what it does "
...
async def get_deadlines(
    ...
    include: Sequence[str] = Query(
        ...
        description=(
            ...
            "the company endpoint's `?include=`. Today only `filings`: one extra "
            "upstream request for the entity's filing history, supplying a real "
            "financial year end where 31 December would otherwise be assumed. Empty "
            "by default; Norway and the United Kingdom accept it and it changes "
            "nothing for them today. Any other value — including one the company "
            "endpoint accepts, such as `charges` — is a `bad_request` (400) naming "
