...
async def get_company(
    ...
    include: Sequence[str] = Query(
        ...
        description=(
            ...
            "and Sweden: the register's own financial figures — turnover, operating "
            "result, profit, balance sheet totals, equity and liabilities — for the "
            "latest filed accounting period, each beside its own currency, answering "
            "whether a supplier looks solvent; Norway's arrive in the same fetch as "
            "`filings`, Sweden's are read out of the entity's own filed annual report; "
            "a `null` figure inside a present block means the company did not report "
            "that line; an absent block means either you did not ask, or the fetch "
            "failed, or — for Sweden — this company has filed no digital annual report, "
            "and the report's own `notes` says which. The United Kingdom does not "
            "declare `financials` at all, so `?include=financials` for `GB` is a "
            "`bad_request`, never an empty or absent block — the accounts of the "
            "companies that matter are filed on paper or as PDF ahead of the "
            "1 April 2028 machine-readable mandate), `lei` (every country except Sweden: "
