...
|---|---|---|---|---|
| T10 | Opus A | review | 2026-09-04: 10-point checklist 8 PASS / 2 FAIL → B2 (`aclose`) and B3 (lifespan `finally`) fixed at 79b18bd / 7eba739; B1/B4 (flaky 429 test, `/mcp` 307) fixed at 7eba739; N1–N8 all applied. New decisions D-011 (employees invariant), D-012 (countries model), D-013 (validate reason), D-014 (`Registry.aclose`). | Sign-off addendum pending |
...
| T14 | Orchestrator | doing | 2026-09-04 clean clone (`git clone . scratchpad/clean`): 273 passed, 1 deselected (live); mypy clean (45 files); ruff clean; wheel stdio → 5 tools; `docker build` ok; `docker compose config` ok with `.env.example`; secret grep empty. `HUMAN_TODO.md` §8 (FIRST_KRONE placeholder) + §9 (launch sequence) written. | Awaiting T10 sign-off, then final commit and feature freeze |
