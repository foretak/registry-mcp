...
- **2026-09-09 ~22:40 local — done.** Kim listed the connector: slug **`io.github.foretak/registry-mcp`**,
  <https://glama.ai/mcp/connectors/io.github.foretak/registry-mcp>, **Healthy**, TDQS **A 4.6/5** across the 7
  tools, last tested 2026-09-09 20:43. The orchestrator pushed the badge as the entry's second line
  (`foretak/awesome-remote-mcp-servers:add-company-check`, `f8b4a35c9d`) and replied on the PR
  (issuecomment-5608872976). CI re-ran: `endpoint-ok` + `has-connector`, mergeable. Glama's TDQS notes, for
  `FEEDBACK.md`: `company_deadlines` lacks a leading verb; the two ChatGPT aliases read as redundant; Sweden's
  no-name-search is recorded as a documented limit, not a failure. The servers listing (§3) shows **0.4.1**
  now, but its description text still reads "Norway … and the UK" — source being traced.
