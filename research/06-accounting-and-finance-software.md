...
**6. Norwegian AML guidance is a specification for `lookup_company`.** Finanstilsynet Rundskriv 15/2019 § 4.4.2: *"Organisasjonsnummer til den juridiske personen skal innhentes og bekreftes."* § 4.4.1: an *oppslag* against Enhetsregisteret/Foretaksregisteret no older than three months, and *"bør opplysningene ikke være eldre enn én måned"*, with *notoritet* (provable record) of the lookup itself. **Read precisely: the three-month figure governs the register lookup itself; the one-month figure governs company information the customer supplies. A 24-hour cache TTL sits well inside both, and `fetched_at` + `source_url` is machine-readable notoritet.** *(Corrected 2026-09-05 after re-reading § 4.4.1.)* [File 08]
