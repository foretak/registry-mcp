...
| `se_ixbrl_currency_unresolved_handbuilt.xhtml` | Hand-built. No real or specimen document measured ever leaves every fact's currency unresolved, so this exists purely to exercise `financials.py::_resolve_currency`'s `None` branch (D-043(d)) — every `ix:nonFraction` fact is tagged `unitRef="PURE"`, a unit whose measure is `xbrli:pure` rather than an `iso4217:CODE` one (REVIEW.md T58 finding 4, mutation M12a). |
