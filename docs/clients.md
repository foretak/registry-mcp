...
The marketplace is [`.claude-plugin/marketplace.json`](../.claude-plugin/marketplace.json); the plugin it points to is [`plugins/registry-mcp/`](../plugins/registry-mcp/), which registers the hosted server via its own `.mcp.json` **and bundles the workflow around it**: a skill ([`skills/company-check/SKILL.md`](../plugins/registry-mcp/skills/company-check/SKILL.md)) that teaches the counterparty check, the filing-deadline check and how to read a register's silences, plus three commands — `/check-supplier` (existence, status and filing health for one identifier, with what it does not establish), `/filing-deadlines` (what is due next, and under which provision) and `/enrich-company-list` (a list of company numbers resolved to one row each, with a source URL per row). Installing the MCP server alone with `claude mcp add` gives you the tools; installing the plugin gives you the tools and the method. Schema and commands: [Plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces) (`name`, `owner.name`, `plugins[].name`, `plugins[].source`, `claude plugin marketplace add <source>`, `claude plugin install <plugin>@<marketplace>`) and [Plugins reference](https://code.claude.com/docs/en/plugins-reference) (`plugin.json`'s `mcpServers` field, pointing at an external `.mcp.json`). Both files pass `claude plugin validate`.
