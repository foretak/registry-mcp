...
**Expected pytest baseline, so a credential gap is not mistaken for a regression.** On this worktree
at `1235ccd` with **no** upstream credentials in the environment, `pytest -q` gives **1,040 passed,
19 failed** — and all nineteen are `test_live_*` in `tests/test_client_gb.py` /
`tests/test_client_se.py`, failing with `upstream_error: This deployment has no … credentials`. Set
`COMPANIES_HOUSE_API_KEY` and `BOLAGSVERKET_CLIENT_ID`/`BOLAGSVERKET_CLIENT_SECRET` from
`~/secrets/registry-mcp/` if you want them green; either way, **the count you record in step 0 is the
count you must still have in step 6**, and none of this task's edits can move it.
