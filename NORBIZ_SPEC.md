...
- **`harRegistrertAntallAnsatte: true` with `antallAnsatte` absent (confirmed on `833285602`, 2026-09-04):** brreg apparently omits `antallAnsatte` rather than sending `0` when the reported count is zero. The current mapper (`registries/no/mapping.py`) reads this as `employees_reported=True, employees=None` — the same shape as "not reported", which §2's own stated intent ("`employees_reported` distinguishes '0 employees' from 'not reported'") does not quite cover for this specific combination. Left as-is per the T03 follow-up that found it (reported, not silently changed) — an unclassified-vs-zero fix here is a mapping-logic decision for the architect, not a spec-text one.
