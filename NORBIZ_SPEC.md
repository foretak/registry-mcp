...
- **`GET /enheter/833286602` → 404, with an empty body.** The build plan names this org.nr as the canonical test number. It is not in the register **and it fails MOD11** (weighted sum 145, remainder 2, so the check digit should be 9, not 2). It is not a valid organisasjonsnummer at all. **T03 must use `923609016` as the live-fetch fixture and `974760673` as the second one.** `833286602` is repurposed as the negative fixture in test 9 below. Raise this with the orchestrator: every done-check in `BRREG_MCP_BUILD_PLAN.md` and `tasks/` that names `833286602` should read `923609016`.
  - **Resolved (2026-09-04, T03 follow-up):** `833286602` was a typo in the build plan. The intended number is **`833285602`** (EL ANSARI KONSULT, `ENK`, Oslo, VAT-registered) — one transposed digit away, and itself MOD11-valid. `GET /enheter/833285602` → 200. Saved as `tests/fixtures/brreg_833285602.json`; it is the **third verified fixture**: a sole proprietorship (`ENK`) with a sparser payload than the `ASA`/`ORGL` fixtures — no `historiskeNavn` entries, no `kapital`, no `stiftelsesdato`, and no `antallAnsatte` despite `harRegistrertAntallAnsatte: true` (see the note on that field below). `833286602` itself remains the correct negative/invalid-MOD11 fixture for test 9; the two numbers are unrelated apart from the typo.
- A 404 body is empty — there is no JSON error document to parse. Do not try.
- **`harRegistrertAntallAnsatte: true` with `antallAnsatte` absent (confirmed on `833285602`, 2026-09-04):** brreg apparently omits `antallAnsatte` rather than sending `0` when the reported count is zero. **Resolved as `DECISIONS.md` D-011** (T10 review item (a)): `employees` always stays `None` when brreg gives no number — never synthesised to `0` — and `employees_reported` is *derived* as `harRegistrertAntallAnsatte and antallAnsatte is not None` rather than mirroring the raw flag, so `employees_reported is True ⟹ employees is not None` is a real invariant. For `833285602` this now yields `employees=None, employees_reported=False`, plus a `notes` entry: "Brønnøysundregistrene flagged an employee count for this entity but did not return the number; treat the employee count as unknown rather than zero." See the updated §2 row below.
...
| `employees` | `antallAnsatte` | `int \| None` | Never synthesised — `None` whenever brreg omits the number, even if `harRegistrertAntallAnsatte` is true (`DECISIONS.md` D-011; confirmed live on `833285602`, which has the flag true with no `antallAnsatte` key at all) |
| `employees_reported` | derived | `bool` | `harRegistrertAntallAnsatte and antallAnsatte is not None` (D-011) — a real invariant: `employees_reported is True ⟹ employees is not None`. When the flag is true but the number is absent, `notes` gets a sentence saying a count was flagged but not returned |
...
- **Deviating accounting years** — *Enhetsregisteret* does not publish the accounting year, so every annual date is a calendar-year assumption. **Corrected 2026-09-05 (R01 §3d, D-023(c)):** Brønnøysundregistrene *does* publish it elsewhere — `GET https://data.brreg.no/regnskapsregisteret/regnskap/{orgnr}` returns `regnskapsperiode: {fraDato, tilDato}` for every filed annual account, open and with no API key (verified live on 923609016 and 982463718). Reading it would let `annual_accounts` pick regnskapsloven § 8-3(1)'s correct branch instead of assuming one. **Shipped 2026-09-08 (D-042(i)'s R-5d, T38):** not as a plain field on `CompanyReport` — D-041(c)'s "one fetch, one `SourceRef`" discipline ruled that shape out — but as `FilingHistory.financial_year_end` behind `include=["filings"]`, the second-round-trip attachment R-5's `include=[…]` mechanism gives every country. `deadlines(report, today)` still stays pure (D-018); nothing in `registries/no/rules.py` reads the attachment yet, so passing `include=["filings"]` to `company_deadlines` does not itself move a Norwegian date off the calendar-year assumption today.
...
- **Roles, board members, sub-unit lists.** **Regnskapsregisteret key figures shipped 2026-09-08 (D-043, T38)** as the `include=["financials"]` attachment — the same fetch as `filings` above, one `SourceRef` shared between the two blocks (D-043(h)) — struck from this DEFERRED list.
...
`not_found` **message**: `"No entity with organisasjonsnummer {id} exists in Enhetsregisteret."` **hint** (N2, T10 review — must not repeat `message`): `"The number is well-formed, so it may never have been issued or the entity may have been deleted. Call search_company with the company name instead."`
